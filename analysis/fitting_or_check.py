...
def poly_fit_and_show(x, y, order, save_path=None, show=True):
    ...
    if show:
        ...
        plt.xlabel('x axis_list')
        plt.ylabel('y axis_list')
...
def linear_regression(x, y, normalize_std_x=True, normalize_std_y=True, show=False):
    ...
    if show:
        print("linear fit: y =", slope, 'x +', intercept, ",   r =", r_value, "   p =", p_value,
              "   std_slope =", std_err_slope)
...
def log_log_linear_fit(scale_list, frequency, cache=10, neglect_ratio=0.001, default=False, show=True):
    ...
    # frequency is freq list [frequency at 1, frequency at 2, ...]
    # neglect_ratio means we overlook freq ratio of instance with largest scales
    if default:
...
def scale_free_check(scale_list, frequency, cache=10, show=True):
    # scale_list is freq ordered list recording the measurements, like area, degree, etc
    # frequency is freq list recording the frequency or probability of each scale
...
def chi2_contigency_test(list_a, list_b, a_level=3, b_level=3):
    ...
    :param list_a: variable freq
    :param list_b: variable b
    :param a_level: level of variable freq
    ...
    for j in range(1000):
        ...
        for i in range(length_a):  # add freq small noise to make every observation distinguishable
...
if __name__ == '__main__':
    ...
    list_object = Functions.pickle_load_object('/home/zhoul0a/Desktop/transfer/temp/clot_volume_branch_level_list.pickle')
    ...
    for item in list_object:
        if item[1] < min_branch:
            min_branch = item[1]
        if item[1] > max_branch:
            max_branch = item[1]
    ...
    list_object = Functions.customized_sort(list_object, compare_fun)
    print(list_object)

    branch_interval_list = []

    cache = 20

    for i in range(cache):
        branch_interval_list.append((max_branch - min_branch) / cache * i + min_branch)
    print(branch_interval_list)

    frequency_array = np.zeros([cache, ], 'float32')

    for item in list_object:
        index = int((item[1] - min_branch) / (max_branch - min_branch) * cache)
        if index == cache:
            index = index - 1
        frequency_array[index] += item[0]

    print(frequency_array)

    frequency_array[-5] = 1023
    frequency_array[-2] = 48

    Functions.show_data_points(branch_interval_list[:-1], np.log(frequency_array + 1)[:-1])

    exit()

    scale_free_check(branch_interval_list, frequency_array, cache=cache)
