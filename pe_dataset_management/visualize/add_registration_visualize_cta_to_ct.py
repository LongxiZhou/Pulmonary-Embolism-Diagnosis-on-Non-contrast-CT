...
from pe_dataset_management.registration.register_cta_to_ct.inference_fast_cta_to_ct import cast_cta_to_non_contrast
...
def images_cta_registered(list_top_dict_reference, scan_name, mass_center=None,
                          flow_dict='/data_disk/CTA-CT_paired-dataset/registration_from_cta_to_non_contrast/optimal'):
    """
    ...
    # registration
    ct_array, airway_mask, lung_mask = cast_cta_to_non_contrast(
        [ct_array, airway_mask, lung_mask], scan_name, flow_top_dict=flow_dict)
    ...
    if os.path.exists(file_path_artery) and os.path.exists(file_path_vein):
        ...
        # registration
        artery_mask, vein_mask, blood_mask = cast_cta_to_non_contrast(
            [artery_mask, vein_mask, blood_mask], scan_name, flow_top_dict=flow_dict)
        ...
        # registration
        blood_mask = cast_cta_to_non_contrast(blood_mask, scan_name, flow_top_dict=flow_dict)
...
def process_scan(scan_name, top_dict='/data_disk/CTA-CT_paired-dataset',
    ...
    image_x, image_y, image_z, mass_center = images_not_register([dict_semantics_non, dict_rescaled_ct_non], scan_name)

    image_x_r, image_y_r, image_z_r, mass_center = images_cta_registered(
        [dict_semantics_cta, dict_rescaled_cta], scan_name, mass_center=mass_center, flow_dict=flow_dict)

    image_x_c, image_y_c, image_z_c, mass_center = images_not_register(
        [dict_semantics_cta, dict_rescaled_cta], scan_name)
...
def process_all(fold=(0, 1)):
    scan_name_list = get_all_scan_name()  # this name will not contain suffix
    scan_name_list = Functions.split_list_by_ord_sum(scan_name_list, fold)
    save_image_dict = '/data_disk/CTA-CT_paired-dataset/' \
                      'registration_from_cta_to_non_contrast/visualization_optimal/semantics_and_ct_registered_3_view'
...
def process_all_twice_register(fold=(0, 1)):
    scan_name_list = get_all_scan_name()[fold[0]:: fold[1]]  # this name will not contain suffix
    save_image_dict = '/data_disk/CTA-CT_paired-dataset/' \
                      'registration_from_cta_to_non_contrast/visualization_twice_register/' \
                      'semantics_and_ct_registered_3_view'
    ...
    for scan_name in Functions.iteration_with_time_bar(scan_name_list):
        if func_check_processed(save_image_dict, scan_name):
            ...
            image_x_combined, image_y_combined, image_z_combined = process_scan(scan_name, flow_dict=flow_top_dict)
            func_file_save(save_image_dict, scan_name, (image_x_combined, image_y_combined, image_z_combined))
...
if __name__ == '__main__':
    ...
    process_scan('Z154')
    exit()
    process_all_twice_register((0, 2))
    exit()
