...
def permutation_test(model, input, ground_truth, params):
    ...
    for e in range(params["permutation_times"] - 1):
        ...
        for i in range(num_of_patients):
            # rand_pred = torch.Tensor(pred[arr[i]]).to(parameters["device"])
            # rand_gt = torch.Tensor(ground_truth[i]).to(parameters["device"])
            # loss = loss + LossByDistance(rand_pred, rand_gt).to(
            #     parameters["device"]).item()
...
def main():
    model = SinLayerNet().to(parameters["device"])
    regularization.train(model, parameters)
    model_dir = os.path.join(parameters["save_model_path"],
                             parameters["save_model_filename"])
    saved_model = torch.load(model_dir)

    input = np.load(parameters["input_dir"])
    gt = np.load(parameters["gt_dir"])
    state = permutation_test(saved_model, input, gt, params=parameters)
    if state:
        print("pass test!")
    else:
