...
import classic_models.Unet_2D.U_net_Model as unm
import classic_models.Unet_2D.U_net_loss_function as unlf
from classic_models.Unet_2D.dataset import RandomFlipWithWeight, RandomRotateWithWeight, ToTensorWithWeight, \
...
def train_loop(model, optimizer, train_loader, test_loader, params=parameters, resume=True):
    ...
    flip_remaining = params["flip_remaining:"]  # initially, model_guided has high recall low precision. Thus, we initially in
    ...
    # when flip_remaining is 0 and the model_guided reached target_performance during recall phase, change to fluctuate phase
