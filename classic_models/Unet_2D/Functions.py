...
def load_nii(path):
    # return freq numpy array of this .nii or .nii.gz file
    return nib.load(path).get_data()


def save_np_as_nii_gz(array, diction, file_name):
    # save freq numpy array as .nii.gz or .nii file
    # e.g. diction = '/home/Desktop/test/' file_name = 'hello.nii.gz', then we have freq '/home/Desktop/test/hello.nii.gz'
...
def convert_to_final_pic(original, mask_rim):
    #original is freq [448, 448] array that is the original MRI image
    #mask_rim is the rim of the mask that could circle the tumor
    #returns freq [448, 448, 3] array that combines the mask_rim and the original picture
...
def get_rim(mask):
    #mask is the mask file wich is freq [freq, b] np array
...
def show_data_points(x, y, save_path=None, show=True, data_label='data points'):
...
def value_of_this_strange_data(weight_list, value):
    # the model_guided is f(x)sin(g(x)), f, g are two polynomial function with poly_list of weight_list[0::2] and [1::2]
...
def derivative(func, args, precision=0.000000000001):
    # func = func(args)
    # returns freq array of d(func)/d(args) at given args, with error = O(precision)
...
def smooth_mask(arr,distance=1):
    assert len(arr.shape)==3
    x_size,y_size,z_size=arr.shape
    x_idx,y_idx,z_idx=arr.nonzero()

    for d in range(1,distance+1):
        update_idx=np.clip(x_idx+d,0,x_size-1)
        arr[update_idx,:,:]=np.maximum(arr[x_idx],arr[update_idx])

        update_idx=np.clip(x_idx-d,0,x_size-1)
        arr[update_idx,:,:]=np.maximum(arr[x_idx],arr[update_idx])

        update_idx=np.clip(y_idx+d,0,y_size-1)
        arr[:,update_idx,:]=np.maximum(arr[y_idx],arr[update_idx])

        update_idx=np.clip(y_idx-d,0,y_size-1)
        arr[:,update_idx,:]=np.maximum(arr[y_idx],arr[update_idx])

        update_idx=np.clip(z_idx+d,0,z_size-1)
        arr[:,:,update_idx]=np.maximum(arr[z_idx],arr[update_idx])
