...
import classic_models.Unet_2D.U_net_Model as Model
...
def load_model(check_point_path, array_info, feature_type=None, gpu=True):
    ...
    key_list = list(array_info.keys())

    if "encoder_blocks" not in key_list:
        array_info["encoder_blocks"] = 4

    encoder_blocks = array_info["encoder_blocks"]
    ...
    if encoder_blocks == 4:
        if feature_type is None:
            ...
            model = Model.UNetCamLast(in_channels=input_channels, out_channels=out_channels,
                                      init_features=array_info['init_features'])
        else:
        ...
        model = Model.UNetLayer3(in_channels=input_channels, out_channels=out_channels,
                                 init_features=array_info['init_features'])
    else:
        assert encoder_blocks == 2
        model = Model.UNetLayer2(in_channels=input_channels, out_channels=out_channels,
                                 init_features=array_info['init_features'])
...
def predict_one_scan_binary_class(rescaled_array, direction, checkpoint_path, array_info, batch_size=16, soft_max=True,
                                  model=None):
    """
    :param model:
    :param soft_max: True for probability for positive class, False for relative certainty for positive class
    ...
    :param checkpoint_path: the check point of the model_guided
    ...
    input_channels = array_info['data_channel'] * len(array_info['window']) + array_info['enhanced_channel']
    sample_array = slicer.slice_one_direction(
        rescaled_array, array_info['resolution'], direction, array_info['data_channel'],
        array_info['enhanced_channel'], array_info['window'],
        positive_semantic_channel=array_info['positive_semantic_channel'], pre_transpose_and_stack=True)

    num_samples = np.shape(sample_array)[0]
    prediction_list = []
    index = 0
    ...
    with torch.no_grad():
        while index < num_samples:
            ...
            if soft_max:
                prediction = soft_max_layer(prediction)
    ...
    return prediction_array[:, :, :, 1] - prediction_array[:, :, :, 0]
...
def predict_one_scan_multi_class(rescaled_array, direction, checkpoint_path, array_info, batch_size=16, evaluate=False,
    ...
    input_channels = array_info['data_channel'] * len(array_info['window']) + array_info['enhanced_channel']
    sample_array = slicer.slice_one_direction(
        rescaled_array, array_info['resolution'], direction, array_info['data_channel'], array_info['enhanced_channel'],
        array_info['window'], positive_semantic_channel=array_info['positive_semantic_channel'],
...
def heat_map_segment(sample, gt, model, gt_channel=1):
    ...
    # inputs = torch.tensor(sample[0: 1, 0: input_channels, :, :], requires_grad=True).float().to("cuda:0")
    prediction = model(inputs)
