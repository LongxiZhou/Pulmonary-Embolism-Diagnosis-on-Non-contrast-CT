...
class UNet3D(nn.Module):
    ...
    def __init__(self, in_channels=2, out_channels=2, init_features=64):
...
class UNet3DSimple(nn.Module):
    ...
    def __init__(self, in_channels=2, out_channels=2, init_features=64):
        ...
        features = init_features
        self.encoder1 = UNet3DSimple._block(in_channels, features, name="enc1")
        self.pool1 = nn.MaxPool3d(kernel_size=2, stride=2)
        self.encoder2 = UNet3DSimple._block(features, features * 2, name="enc2")
        self.pool2 = nn.MaxPool3d(kernel_size=2, stride=2)
        self.encoder3 = UNet3DSimple._block(features * 2, features * 4, name="enc3")
        self.pool3 = nn.MaxPool3d(kernel_size=2, stride=2)

        self.bottleneck = UNet3DSimple._block(features * 4, features * 8, name="bottleneck")

        self.upconv3 = nn.ConvTranspose3d(
            features * 8, features * 4, kernel_size=(2, 2, 2), stride=(2, 2, 2)
        )
        self.decoder3 = UNet3DSimple._block((features * 4) * 2, features * 4, name="dec3")
        self.upconv2 = nn.ConvTranspose3d(
            features * 4, features * 2, kernel_size=(2, 2, 2), stride=(2, 2, 2)
        )
        self.decoder2 = UNet3DSimple._block((features * 2) * 2, features * 2, name="dec2")
        self.upconv1 = nn.ConvTranspose3d(
            features * 2, features, kernel_size=(2, 2, 2), stride=(2, 2, 2)
        )
        self.decoder1 = UNet3DSimple._block(features * 2, features, name="dec1")
...
class UNet3DSimplest(nn.Module):
    ...
    def __init__(self, in_channels=2, out_channels=2, init_features=64):
        ...
        features = init_features
        self.encoder1 = UNet3DSimplest._block(in_channels, features, name="enc1")
        self.pool1 = nn.MaxPool3d(kernel_size=2, stride=2)
        self.encoder2 = UNet3DSimplest._block(features, features * 2, name="enc2")
        self.pool2 = nn.MaxPool3d(kernel_size=2, stride=2)

        self.bottleneck = UNet3DSimplest._block(features * 2, features * 4, name="bottleneck")

        self.upconv2 = nn.ConvTranspose3d(
            features * 4, features * 2, kernel_size=(2, 2, 2), stride=(2, 2, 2)
        )
        self.decoder2 = UNet3DSimplest._block((features * 2) * 2, features * 2, name="dec2")
        self.upconv1 = nn.ConvTranspose3d(
            features * 2, features, kernel_size=(2, 2, 2), stride=(2, 2, 2)
        )
        self.decoder1 = UNet3DSimplest._block(features * 2, features, name="dec1")
...
def block_cnn(in_channels, features, name, leaky=0.1):
