...
feature_list: is a list of names: ['SGRQ', 'invisible_lesion', ...]
data_array: a array with shape (number_samples, len(feature_list))
...
select_features(target_column, candidate_feature_list=None, importance_rank=None, baseline_r=None, encoding_depth=0,
                    leave_count=6)
e.g.
...
if prognosis:
    ...
    #data_set = Functions.pickle_load_object('/home/zhoul0a/Desktop/prognosis_project/data_access/oxy_index_dataset_interpolate.pickle')
...
def xg_regression(target_column, data_column_list, test_num=1, evaluate=True, save_report_path=None, show=True,
    ...
    if show:
        ...
        Functions.show_data_points(gt, predicted, x_name='Ground Truth', y_name='Predicted Value', title='Prediction of PaO2/FiO2 ratio', data_label=None,
                                   save_path='/home/zhoul0a/Desktop/Predict_PaO2_FiO2.svg')
    if not difference:
...
def select_features(target_column, candidate_feature_list=None, importance_rank=None, baseline_r=None, depth=0,
    ...
    :param depth: too track the recursive encoding_depth
    :param baseline_r: r_score using all_file candidate features
...
if __name__ == '__main__':
    ...
    if prognosis:
        ...
        xg_regression(1, [17, 16, 23], difference=True)

    else:

        xg_regression(0, [5, 4, 10, 6, 3, 2], difference=True)
