...
from pulmonary_embolism_v3.utlis.data_augmentation import random_flip_rotate_swap_sample, get_labels
from segment_clot_cta.prepare_training_dataset.simulate_clot_rescaled_cta import get_default_func_reset_ct


def apply_clot_on_sample(sample, list_clot_sample_dict,
                         min_volume=250, max_trial=10, augment=True, visualize=False, trace_clot=True,
    ...
    global_bias = random.uniform(global_bias_range[0], global_bias_range[1])

    func_apply_clot = partial(apply_clot_on_sample_sequence_parallel, func_reset_ct=None, send_end=None,
                              augment_labels=None, lesion_id=None, augment=False, return_sample=True, deep_copy=False,
    ...
    sample_sequence_with_clot, lesion_id_volume_dict = apply_multiple_clot(
        ...
        min_volume=min_volume, max_trial=max_trial, remain_unqualified=True, trace_clot=trace_clot)
...
def apply_clot_on_sample_sequence_parallel(sample, clot_sample_dict, func_reset_ct=None, send_end=None,
                                           augment=True, augment_labels=None, lesion_id=None, deep_copy=True,
                                           return_sample=False, global_bias=0.):
    """
    ...
    if func_reset_ct is None:
        ct_value_list = get_value_list_clot(potential_item_list, new_clot_loc_set)
        max_clot_depth = clot_depth_dict['max_depth']
        noise = random.randint(0, 80) / 1600
        func_reset_ct = get_default_func_reset_ct(ct_value_list, max_clot_depth, noise_cta=noise,
                                                  global_bias=global_bias)

    if func_reset_ct is None:  # this means len(ct_value_list) == 0
        num_clot_voxels = 0
    else:
        num_clot_voxels = reset_ct_value_and_establish_clot_array(
            potential_item_list, new_clot_loc_set, clot_depth_dict, center_location_clot, func_reset_ct, lesion_id)
...
def apply_multiple_clot(sample, list_clot_sample_dict, func_apply_clot, min_volume=250, max_trial=10,
                        ...
                        return_id_volume_dict=False, trace_clot=True):
    """
    ...
    for clot_sample_dict in list_clot_sample_dict:
        if trace_clot:
            current_lesion_id = lesion_id
        else:
            current_lesion_id = None
        sample_copy, num_clot_voxels = apply_clot_on_sample_until_satisfy(
            sample_copy, clot_sample_dict, func_apply_clot, current_lesion_id, min_volume, max_trial,
            remain_unqualified)
...
def reset_ct_value_and_establish_clot_array(potential_item_list, loc_clot_set, clot_depth_dict, center_location_clot,
                                            func_reset_ct, lesion_id):
    """
    ...
    """
    assert len(potential_item_list) > 0
    assert lesion_id is None or lesion_id >= 1
    ...
    def process_one_item(item_input):
        ...
        blood_region = item_input['blood_region']
        loc_set_sample = set()
        ...
        if len(intersection_loc_set) > 0:
            ...
            for location in intersection_loc_set:
                ...
                if blood_region[local_loc] < 0.5:  # not blood region
                    continue
                ...
                ct_cube[local_loc] = func_reset_ct(clot_depth)
                clot_array[local_loc] = 1  # clot_mask
                if lesion_id is not None:
...
def get_value_list_clot(potential_item_list, loc_clot_set):
    """
    ...
    def process_one_item(item_input):
        ...
        if len(intersection_loc_set) > 0:
            for location in intersection_loc_set:
                ...
                if blood_region[local_loc] < 0.5:  # not blood region
                    continue
                ct_value_list.append(ct_cube[local_loc])

    for item in potential_item_list:
        process_one_item(item)

    return ct_value_list


def visualize_clots_on_sample_sequence(sample_sequence_with_clot, high_resolution=True, clip_window=False, z_start=0):
    ...
    clot_array = converter.reconstruct_rescaled_ct_from_sample_sequence(
    ...
    blood_region = converter.reconstruct_rescaled_ct_from_sample_sequence(
        sample_sequence_with_clot, absolute_cube_length, key='blood_region')

    blood_region = np.array(blood_region > 0, 'float32')

    print("augmented blood region")
    stl.visualize_numpy_as_stl(blood_region)

    ct_array = converter.reconstruct_rescaled_ct_from_sample_sequence(
...
def check_lesion_apply(high_resolution=True, global_bias_range=(-0., 0.)):
    reduced_clot_sample_list = Functions.pickle_load_object(
        '/data_disk/pulmonary_embolism/segment_clot_on_CTA/clot_pickle/merge_reduced/merged_shuffled_5%.pickle')
    ...
    sample_test = Functions.pickle_load_object(
        ...
        'pe_v3_long_length_complete_vessel/denoise_low-resolution/AL00003.pickle'
    )

    sample_sequence_with_clot = apply_clot_on_sample(sample_test, temp_clot_sample_list,
                                                     visualize=True, trace_clot=True,
                                                     global_bias_range=global_bias_range, min_volume=50)
    visualize_clots_on_sample_sequence(sample_sequence_with_clot, high_resolution=high_resolution)


if __name__ == '__main__':
    random.seed = 1
    check_lesion_apply(high_resolution=False, global_bias_range=(-0.2, -1))
    exit()

    sample_annotated = Functions.pickle_load_object(
        '/data_disk/pulmonary_embolism/segment_clot_on_CTA/sample_sequence/PE_with_gt/next_82/'
        'pe_v3_long_length_complete_vessel/original_low-resolution/11.17p04.pickle')['sample_sequence']

    visualize_clots_on_sample_sequence(sample_annotated, high_resolution=False, clip_window=True)
