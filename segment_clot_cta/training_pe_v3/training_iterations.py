...
import segment_clot_cta.training_pe_v3.dataset_and_dataloader as dataset_and_loader
...
def training(params):
    ...
    train_loader = dataset_and_loader.DataLoaderSimulatedClot(
        ...
        global_bias_range=params["global_bias_range"])
    ...
    train_loader.update_clot_simulation_parameter(global_bias_range=params["global_bias_range"])
    test_loader.update_clot_simulation_parameter(global_bias_range=params["global_bias_range"])
...
def train_loop(model, optimizer, train_loader, test_loader, params=None):
    ...
    train_loader.update_clot_simulation_parameter(global_bias_range=global_bias_range)
    test_loader.update_clot_simulation_parameter(global_bias_range=global_bias_range)
    ...
    for epoch in range(epoch_start + 1, epoch_start + 1 + params["n_epochs"]):
        ...
        if flip_remaining == 1:
            ...
            # if abs(global_bias_range[0]) >= 0.001:  # 1.6 HU
            ...
            # global_bias_range[1] = global_bias_range[0] * 5
            train_loader.update_clot_simulation_parameter(global_bias_range=global_bias_range)
            test_loader.update_clot_simulation_parameter(global_bias_range=global_bias_range)
