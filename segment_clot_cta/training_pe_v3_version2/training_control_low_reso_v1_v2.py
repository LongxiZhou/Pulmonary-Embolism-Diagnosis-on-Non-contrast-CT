...
parameters = {
    ...
    "flip_recall": 0.5,  # reach this recall, then can flip
    "flip_precision": 0.5,  # reach this precision, then can flip
    ...
    'min_dice_at_flip': 0.3,  # reach this dice at flip (recall == precision),
    ...
    "min_clot": 50,  # the volume of clot when applied to vessel, 300 for high resolution, 50 for low resolution

    # simulation function 1: direct change the signal for clots
    "global_bias_range": None,  # set to None if you want to resume previous model's global_bias_range;
    # start from large minus like (-0.2, -1);
    # stable at (0, 0)

    # simulation function 2: add shift for clot regions
    "value_increase": [-0.025, -0.125],  # set to None if you want to resume previous model's value_increase;
    ...
    "sample_dir_list_with_gt": ["/data_disk/pulmonary_embolism/segment_clot_on_CTA/PE_CTA_with_gt/sample_sequence/"
                                "pe_v3_long_length_complete_vessel/"
                                "denoise_low-resolution",
                                "/data_disk/pulmonary_embolism/segment_clot_on_CTA/PE_CTA_with_gt/sample_sequence/"
                                "pe_v3_long_length_complete_vessel/"
                                "original_low-resolution"],

    # frequency for different clot simulation approach (will be normalize)
    "relative_frequency_v1_v2": [1, 1],  # (simulation_function 1, simulation_function 2)
    # frequency for different clot ground truth (will be normalize)
    "relative_frequency_simulate_gt": [1, 0],  # (simulated_clot, annotated_clot)
