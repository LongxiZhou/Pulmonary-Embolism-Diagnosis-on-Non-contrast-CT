...
from segment_clot_cta.prepare_training_dataset.simulate_clot_pe_v3 import random_select_clot_sample_dict
import segment_clot_cta.prepare_training_dataset.simulate_clot_pe_v3 as simulation_v1
import pulmonary_embolism_v3.prepare_training_dataset.simulate_clot as simulation_v2
...
class DataLoaderSimulatedClot:
    ...
    def __init__(self, clot_dataset, original_sample_dataset, batch_size, shuffle=False,
                 num_workers=8,
                 ...
                 global_bias_range=(-0., 0.), annotated_sample_dataset=None, relative_frequency_v1_v2=None,
                 relative_frequency_simulate_gt=None):
        """

        :param clot_dataset: instance of ClotDataset:
        :param original_sample_dataset: instance of SampleDataset, formed by non-clot CTA
        :param annotated_sample_dataset: instance of SampleDataset, formed by clot CTA with annotation
        ...
        """
        self.relative_frequency_simulate_gt = relative_frequency_simulate_gt
        if self.relative_frequency_simulate_gt is None:  # only use simulation
            self.relative_frequency_simulate_gt = np.array([1, 0], 'float32')

        self.relative_frequency_simulate_gt = self.relative_frequency_simulate_gt / np.sum(
            self.relative_frequency_simulate_gt)

        print("relative_frequency for simulation and annotation:", self.relative_frequency_simulate_gt)

        self.relative_frequency_v1_v2 = relative_frequency_v1_v2
        if self.relative_frequency_v1_v2 is None:  # equally use simulation v1 and v2
            self.relative_frequency_v1_v2 = np.array([1, 1], 'float32')

        self.relative_frequency_v1_v2 = self.relative_frequency_v1_v2 / np.sum(
            self.relative_frequency_v1_v2)

        print("relative_frequency for simulation_v1 for simulation_v2:", self.relative_frequency_v1_v2)
        ...
        self.annotated_sample_dataset = annotated_sample_dataset
        self.epoch_passed = 0
        ...
        self.add_clot_on_sample_v1 = partial(simulation_v1.apply_clot_on_sample, trace_clot=self.trace_clot,
                                             min_volume=self.min_clot, max_trial=10, augment=augment, visualize=False,
                                             global_bias_range=self.global_bias_range)

        self.add_clot_on_sample_v2 = partial(simulation_v2.apply_clot_on_sample, func_change_ct=self.func_change_ct,
    ...
    def update_clot_simulation_parameter_v1(self, global_bias_range=None, min_clot=None, max_trial=10,
    ...
    def update_clot_simulation_parameter_v2(self, power_range, add_base_range, value_increase, voxel_variance,
    ...
    def get_input_list_for_sub_process_func(self, send_end):
        ...
        for i in range(self.batch_size):
            ...
            if random.uniform(0, 0.999999) < self.relative_frequency_simulate_gt[0]:  # simulate sample
                ...
                sample, whether_important = self.annotated_sample_dataset.get_item()  # use annotated sample
                list_clot_sample_dict = []  # no clot to input means do not simulate any clot
    ...
    def sub_process_func(self, *input_list):
        ...
        for sample, list_clot_sample_dict, whether_important in value_list:
            if random.uniform(0, 0.999999) < self.relative_frequency_v1_v2[0]:
                list_sample_sequence.append(self.add_clot_on_sample_v1(sample, list_clot_sample_dict))
            else:
                list_sample_sequence.append(self.add_clot_on_sample_v2(sample, list_clot_sample_dict))
    ...
    def __len__(self):
        ...
        # if non-clot CT is complete used, we end the iteration
        augment_ratio = sum(self.relative_frequency_simulate_gt) / self.relative_frequency_simulate_gt[0]
        samples_num = int(samples_num * augment_ratio)
    ...
    def __iter__(self):
        ...
        if self.shuffle:
            ...
            self.annotated_sample_dataset.shuffle()
