...
def visualize_prediction_for_one_file(top_dict='/data_disk/pulmonary_embolism/segment_clot_on_CTA/PE_CTA_with_gt/',
                                      file_name='patient-id-135', gt=False, loaded_model=None, trim=True):
    """
    ...
    sample_sequence_ = Functions.pickle_load_object(
        top_dict + 'sample_sequence/pe_v3_long_length_complete_vessel/denoise_low-resolution/' +
        file_name + '.pickle')["sample_sequence"]

    sample_sequence_ = predict_clot_for_sample_sequence(
        sample_sequence_, model=loaded_model, high_resolution=False, trim=trim)
    probability_mask = converter.reconstruct_rescaled_ct_from_sample_sequence(sample_sequence_,
                                                                              key='clot_prob_mask',
    ...
    for z in z_list[::2]:
        ...
        Functions.image_save(image, top_dict + 'visualization/predict_clot_add_supervise (low_reso_complete_vessel)/' +
                             file_name + '/' + str(z) + '.png',
                             dpi=300)
...
if __name__ == '__main__':
    ...
    for fn in fn_list[1::2]:
        visualize_prediction_for_one_file(
            file_name=fn[:-4], gt=True, loaded_model=load_saved_model_guided(
                high_resolution=False, model_path='/data_disk/pulmonary_embolism/segment_clot_on_CTA/check_point/'
                                                  'good_models/low_resolution_complete_vessel_long_v1_gt/'
                                                  'best_model_guided.pth'))
