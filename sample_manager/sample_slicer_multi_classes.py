...
further fine-tuning of our three-way model_guided, we may input the intermediate results of other directions, which increases 
...
slice_index: it is the id of the image that the model_guided aimed to segment. Your model_guided input a sample, and output a 
...
if the model_guided input is [Batch_Num, C, H, W], then [C, H, W] is [data_channel * len(window) + enhanced_channel, 512, 512]
...
def slice_one_sample(rescaled_array, resolution, slice_index, direction, data_channel, enhanced_channel, window):
    ...
    if length > slice_index >= 0:  # slice_index is the id of the image that the model_guided aimed to segment.
        if direction == 'X':
...
def slice_one_direction(rescaled_array, resolution, direction, data_channel, enhanced_channel, window,
                        ...
                        pre_transpose_and_stack=True):
    # we will call this function both in training and testing.
    # the rescaled_array has shape: [512, 512, 512, data_channel + enhanced_channel + semantic_channel], in 'float32'
    # window refers to the physical distance in mm
    # during training, sometimes we want to neglect the sample if the segmentation map is uniform (like all_file pixel
    # corresponds to only one class), thus neglect_negative=True; during testing, we need to slice all_file slices, thus
    ...
    if len(shape) == 3:
        shape = list(shape)
        shape.append(1)
        rescaled_array = np.reshape(rescaled_array, shape)
    semantic_channel = shape[3] - data_channel - enhanced_channel
    ...
    for slice_index in range(length):
        ...
        if train_period and neglect_negative and neglect_interval is not None:
            ...
            if lesion_pixels <= threshold:  # this means this sample only contain normal tissue
                continue
        if pre_transpose_and_stack:
            sample = np.transpose(sample, (2, 0, 1))

        if slice_index == 0:
            sample_array = np.zeros([length, np.shape(sample)[0], np.shape(sample)[1], np.shape(sample)[2]], 'float32')

        if pre_transpose_and_stack:
            sample_array[slice_index] = sample
        else:
            sample_list.append(sample)
...
def prepare_training_set(dict_for_rescaled_array, save_dict, data_channel, enhanced_channel, positive_semantic_channel,
    ...
    # positive_semantic_channel is for neglect all_file normal slices.
    ...
    for rescaled_array_name in rescaled_array_name_list:
        ...
        for sample in sample_list_x:
            save_count += 1
            Functions.save_np_array(save_dict + 'X/', 'X_' + str(save_count) + '_' + rescaled_array_name[:-4], sample)
        save_count = 0
        for sample in sample_list_y:
            save_count += 1
            Functions.save_np_array(save_dict + 'Y/', 'Y_' + str(save_count) + '_' + rescaled_array_name[:-4], sample)
        save_count = 0
        for sample in sample_list_z:
            save_count += 1
            Functions.save_np_array(save_dict + 'Z/', 'Z_' + str(save_count) + '_' + rescaled_array_name[:-4], sample)
        scan_left -= 1
...
def prepare_training_set_one_direction(dict_for_rescaled_array, save_dict, data_channel, enhanced_channel,
                                       positive_semantic_channel, resolution=(1, 1, 1), window=(-1, 0, 1), threshold=0,
                                       neglect_interval=None, direction='Z'):
    # param dict_for_rescaled_array: the directory where rescaled arrays are stored. can be .npz or .npy
    # param save_dict: the directory where the training samples will be stored
    # data_channel, enhanced_channel are both int, means their number
    # positive_semantic_channel is a list, indicating the lesion channel in semantic_channels, like (1, 2, 3).
    ...
    report = open(save_dict + 'report.txt', 'a')
    report.write('scan id, sample_num_' + direction + '\n')
    scan_left = num_scans
    for rescaled_array_name in rescaled_array_name_list:
        print(scan_left, 'number of scans waiting to slicing')
        if rescaled_array_name[-1] == 'y':  # this means is .npy file
        ...
        if direction == 'X':
            sample_list = slice_one_direction(rescaled_array, resolution, 'X', data_channel, enhanced_channel, window,
                                              True, positive_semantic_channel, threshold, neglect_interval)
        elif direction == 'Y':
            sample_list = slice_one_direction(rescaled_array, resolution, 'Y', data_channel, enhanced_channel, window,
                                              True, positive_semantic_channel, threshold, neglect_interval)
        else:
            sample_list = slice_one_direction(rescaled_array, resolution, 'Z', data_channel, enhanced_channel, window,
                                              True, positive_semantic_channel, threshold, neglect_interval)
        print('scan', rescaled_array_name, 'has:', len(sample_list), 'samples')

        save_count = 0

        for sample in sample_list:
            save_count += 1
            Functions.save_np_array(save_dict + direction + '/', direction + '_' + str(save_count) + '_' +
                                    rescaled_array_name[:-4], sample)

        scan_left -= 1
        report.write(rescaled_array_name + ' ' + str(len(sample_list)))
        report.write('\n')
    report.close()
...
def prepare_training_set_v2(dict_for_rescaled_array, dict_for_rescaled_gt, gt_channel, save_dict, data_channel,
    ...
    for rescaled_array_name in rescaled_array_name_list:
        ...
        if rescaled_array_name[-1] == 'y':  # this means is .npy file
            ...
            print('illegal filename for:', rescaled_array_name)
            raise ValueError('rescaled_array must be .npy or .npz file')
        gt_array = np.load(dict_for_rescaled_gt + rescaled_array_name[:-4] + '.npz')['array']
        if len(np.shape(gt_array)) == 4:
            gt_array = gt_array[:, :, :, gt_channel]
...
if __name__ == '__main__':
    ...
    prepare_training_set_v2('/home/zhoul0a/Desktop/Feb/rescaled_ct/',
                            '/home/zhoul0a/Desktop/Feb/rescaled_gt_2/', 0,
                            '/home/zhoul0a/Desktop/Feb/training_sample/', 1, 0, (0,))
    exit()
    import sys
    sys.path.append('/ibex/scratch/projects/c2052/Lung_CAD_NMI/source_codes')

    source_dict = '/home/zhoul0a/Desktop/absolutely_normal/arrays_raw_temp'
    target_dict = '/home/zhoul0a/Desktop/absolutely_normal/training_samples'
    prepare_training_set(source_dict, target_dict, 1, 0, (0,), threshold=5)

    exit()
    import models.Unet_2D.training_control

    exit()
    source_dict = '/home/zhoul0a/Desktop/Lung_CAD_NMI/raw_data/blood_vessel/arrays_raw_with_enhanced_channel'
    target_dict = '/home/zhoul0a/Desktop/Lung_CAD_NMI/raw_data/blood_vessel/training_samples_enhanced'
    prepare_training_set(source_dict, target_dict, 1, 2, (0,))
    exit()

    import Tool_Functions.Functions as Functions

    array = np.load('/home/zhoul0a/Desktop/transfer/test/weights_Z_101_xwqg-A00126_2019-08-15.npy')
    for i in range(2):
        Functions.image_show(array[:, :, i])
        #Functions.merge_image_with_mask(array[:, :, i], array[:, :, 5])
