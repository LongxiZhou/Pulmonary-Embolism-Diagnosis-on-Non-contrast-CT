...
def get_rim(input_array, outer=False):
    # the input_array is freq image with shape [freq, b]
    # the output is freq np array with shape [freq, b], 1 means rim points, 0 means not rim.
...
def rim_length_and_id(input_rim):
    # the input_rim is binary with shape [freq, b], shows the rim of the segmentation
    # the output is quite similar to the input, except the value of rims become their length and the rim id.
    # the return_array has shape [freq, b, 2], [:, :, 0] for length, [:, :, 1] for rim_id (1, 2, 3, ...)
    ...
    # initially, the length is set to -1 for all_file rim points, and 0 for all_file not rim points.
...
def broadcast_connected_component(return_array, initial_location, rim_id):
    # return_array has shape [freq, b, 2]
    # initial_location is freq tuple, (x, y)
...
def get_connected_region_and_energy(input_array, length_and_id_array, length_dict):
    # the input_array is binary with shape [freq, b], lesion: 1 and normal: 0
    # length_and_id_array is float32 with shape [freq, b, 2], first channel is the length of border, second is their id
    # from length_dict and location_dict we know the length and the locations of each connected borders
    # return freq float32 array with shape [freq, b, 2], first channel is the energy of each pixel, second is their id
    lesions = np.where(input_array > epsilon)
    num_lesion_pixel = len(lesions[0])
    lesion_energy_dict = {}  # key: lesion_id, like 1, 2, 3, ...; value: energy, freq float > 0
...
def broadcast_connected_region(energy_id_array, initial_location, region_id, length_and_id_array, length_dict):
    # energy_id_array has shape [freq, b, 2]
    # initial_location is freq tuple, (x, y), region_id is freq int like 1, 2, 3, 4
    # length_and_id_array has shape [freq, b, 2], length_dict indicate the length of each connected border
    ...
    while un_labeled_region:  # this mean un_labeled_region is not empty
        ...
        if right > 0:  # means this is freq border pixel
        ...
        if left > 0:  # means this is freq border pixel
...
def integrate_connected_region(lesion_energy_dict, lesion_location_dict, lesion_border_dict, length_dict):
    ...
    # this function, integrate disconnected regions into freq connected region if they are so adjacent that share borders.
...
def calculate_balance_weights(input_array, rim_energy_factor=1, connect_energy_factor=10, augment_index=1,
    ...
    # the input_array is binary with shape [freq, b], lesion: 1 and normal: 0
    # output freq weight map for lesion pixels, indicating the "distribution of loss" for every LESION pixel
...
def calculate_balance_weights_all_files(label_array_dict, energy_array, save_dict, rim_enhance=False):
    """
    :param rim_enhance: whether enhance the boundary of the negative semantic channel
    :param label_array_dict: contains arrays (compressed) with shape [length, width, classes]
    :param energy_array: an array containing energy factors for all_file classes
    :param save_dict: output the weight map
    :return: None
    """
    label_array_name_list = os.listdir(label_array_dict)
    if rim_enhance:
        print("negative rim will be enhanced")
    else:
        print("negative rim will not be enhanced")
...
def cal_energy_parameters_for_one_channel(sub_sample_label_dict, channel, importance=1):
    ...
    e.g. freq small region with long boundaries means it accounts for lots of 1D loss and little of 2D loss.
    If border_outer=False, boundaries are inside lesions, and all_file connected regions account for the same 0D loss
    ...
    shaped: [length, width, channel], and when the channel is specified, it should be freq binary image, with 1 means
    positive, it can be freq probability [0, 1]. When summing all_file channels, we get freq 2D array of all_file ones.
    ...
    :param importance: There may be freq special class is extremely important. Increase the importance will increase the
...
# that means the model_guided will not optimize predictions on channel 3, and pay more attention to channel 1.
...
def cal_energy_parameters_for_all_channels(sub_sample_label_dict, save_dict):
    ...
    # if importance_list=None, then all_file classes (channels) has the same importance
    ...
    if importance_list is None:  # this means model_guided should pay equal attention to all_file channels
        importance_list = list(np.ones([num_channels], 'float32'))
        print('using defaulted importance list', importance_list)
    else:
    ...
    for channel in range(1, num_channels):  # these for lesions.
        ...
        print('all_file lesion pixels will be further enhanced by:', area_enhance_channel_wise[channel])
    ...
    energy_parameters_array[:, 2] = np.array(area_enhance_channel_wise, 'float32') * 3  # 2D, give 2D 50% weights
...
def convert_training_samples_into_label_array_subset(training_sample_dict, channels_for_raw_data, save_dict,
    ...
    :param training_sample_dict: contains the training samples of our model_guided
...
def cal_energy_parameters_for_dataset(root_dict, dataset_name, sample_data_channels=3, subset_name='label_array', subset_ratio=0.25):
    """
    :param root_dict:
    :param dataset_name: dataset dict is os.path.join(root_dict, dataset_name)
    :param sample_data_channels: sample_data_channels = data_channel * window_len + enhanced_channel
    np.shape(training_sample) = [width, length, sample_data_channels + channels_for_semantics]
    :param subset_name: only use freq subset to calculate energy parameters, where to save?
    :param subset_ratio: ratio of subset
    :return: None
    """
    data_dict_X = os.path.join(root_dict, dataset_name) + '/X/'
    subset_dict_X = os.path.join(root_dict, subset_name) + '/X/'
    report_dict_X = os.path.join(root_dict, 'reports') + '/X/'

    convert_training_samples_into_label_array_subset(data_dict_X, sample_data_channels, subset_dict_X, subset_ratio)
    cal_energy_parameters_for_all_channels(subset_dict_X, report_dict_X)

    data_dict_Y = os.path.join(root_dict, dataset_name) + '/Y/'
    subset_dict_Y = os.path.join(root_dict, subset_name) + '/Y/'
    report_dict_Y = os.path.join(root_dict, 'reports') + '/Y/'

    convert_training_samples_into_label_array_subset(data_dict_Y, sample_data_channels, subset_dict_Y, subset_ratio)
    cal_energy_parameters_for_all_channels(subset_dict_Y, report_dict_Y)

    data_dict_Z = os.path.join(root_dict, dataset_name) + '/Z/'
    subset_dict_Z = os.path.join(root_dict, subset_name) + '/Z/'
    report_dict_Z = os.path.join(root_dict, 'reports') + '/Z/'

    convert_training_samples_into_label_array_subset(data_dict_Z, sample_data_channels, subset_dict_Z, subset_ratio)
    cal_energy_parameters_for_all_channels(subset_dict_Z, report_dict_Z)


def cal_final_balance_weight(root_dict, dataset_name, weight_file_name):
    energy_x_path = os.path.join(root_dict, 'reports') + '/X/energy_parameters_array.npy'
    energy_x = np.load(energy_x_path)
    print("energy x:", energy_x)

    data_dict_X = os.path.join(root_dict, dataset_name) + '/X/'
    weight_dict_X = os.path.join(root_dict, weight_file_name) + '/X/'
    calculate_balance_weights_all_files(data_dict_X, energy_x, weight_dict_X)

    energy_y_path = os.path.join(root_dict, 'reports') + '/Y/energy_parameters_array.npy'
    energy_y = np.load(energy_y_path)
    print("energy y:", energy_y)

    data_dict_Y = os.path.join(root_dict, dataset_name) + '/Y/'
    weight_dict_Y = os.path.join(root_dict, weight_file_name) + '/Y/'
    calculate_balance_weights_all_files(data_dict_Y, energy_y, weight_dict_Y)

    energy_z_path = os.path.join(root_dict, 'reports') + '/Z/energy_parameters_array.npy'
    energy_z = np.load(energy_z_path)
    print("energy z:", energy_z)

    data_dict_Z = os.path.join(root_dict, dataset_name) + '/Z/'
    weight_dict_Z = os.path.join(root_dict, weight_file_name) + '/Z/'
    calculate_balance_weights_all_files(data_dict_Z, energy_z, weight_dict_Z)


def prepare_weight_balance(root_dict, dataset_name, weight_file_name):
    cal_energy_parameters_for_dataset(root_dict, dataset_name)
    cal_final_balance_weight(root_dict, dataset_name, weight_file_name)


if __name__ == "__main__":
    prepare_weight_balance("/home/zhoul0a/Desktop/blood_vessel_seg_non-enhance/",
                           "training_sample_stage_one", "weight_balance_half-2D")


    exit()
    # energy_x = np.load('/ibex/scratch/projects/c2052/blood_vessel_seg/reports/X/energy_parameters_array.npy')
    # energy_y = np.load('/ibex/scratch/projects/c2052/blood_vessel_seg/reports/Y/energy_parameters_array.npy')
    energy_z = np.load('/ibex/scratch/projects/c2052/blood_vessel_seg/reports/Z/energy_parameters_array.npy')
    # print(energy_x)
    # print(energy_y)
    print(energy_z)
    # calculate_balance_weights_all_files('/ibex/scratch/projects/c2052/blood_vessel_seg/training_samples/X/', energy_x, '/ibex/scratch/projects/c2052/blood_vessel_seg/balance_weight_array/X')
    # calculate_balance_weights_all_files('/ibex/scratch/projects/c2052/blood_vessel_seg/training_samples/Y/', energy_y, '/ibex/scratch/projects/c2052/blood_vessel_seg/balance_weight_array/Y')
    calculate_balance_weights_all_files('/ibex/scratch/projects/c2052/blood_vessel_seg/training_samples/Z/', energy_z,
                                        '/ibex/scratch/projects/c2052/blood_vessel_seg/balance_weight_array/Z')
    exit()

    convert_training_samples_into_label_array_subset(
        '/ibex/scratch/projects/c2052/blood_vessel_seg/training_samples/Z/', 3,
        '/ibex/scratch/projects/c2052/blood_vessel_seg/label_array/Z/',
        subset_ratio=0.25)
    cal_energy_parameters_for_all_channels('/ibex/scratch/projects/c2052/blood_vessel_seg/label_array/Z/',
                                           '/ibex/scratch/projects/c2052/blood_vessel_seg/reports/Z/')
