    ...
    for epoch in range(epoch_start + 1, epoch_start + 1 + params["n_epochs"]):
        ...
        for i, batch_sample in enumerate(train_loader):
            ...
            loss = loss_function.weighted_absolute_difference_loss_tissue_wise(
                tensor_predict, tensor_gt, tensor_penalty)
            optimizer.zero_grad()
            loss.backward()
            optimizer.step()
    ...
    with torch.no_grad():
        ...
        for i, batch_sample in enumerate(test_loader):
            ...
            loss_batch = loss_function.weighted_absolute_difference_loss_tissue_wise(
                tensor_predict, tensor_gt, tensor_penalty, mean=True)
...
def training(params):
    ...
    train_loader = dataset_and_loader.DataLoaderForPE(train_dataset, params["batch_ct"], params["batch_split"],
    ...
    test_loader = dataset_and_loader.DataLoaderForPE(test_dataset, params["batch_ct"], params["batch_split"],
                                                     params["ratio_mask_input"], params["ratio_predict"],
