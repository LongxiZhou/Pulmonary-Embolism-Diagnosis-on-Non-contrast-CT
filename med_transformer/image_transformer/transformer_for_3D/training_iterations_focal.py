...
def train_loop(model, optimizer, train_loader, test_loader, params=None):
    ...
    for epoch in range(epoch_start + 1, epoch_start + 1 + params["n_epochs"]):
        ...
        for i, batch_sample in enumerate(train_loader):
            ...
            tensor_gt, tensor_penalty = loss_function.form_tensors_tissue_wise_v2(
...
def evaluate(model, test_loader, params, show=True):
    ...
    with torch.no_grad():
        ...
        for i, batch_sample in enumerate(test_loader):
            ...
            values["voxel_differ_avg (HU)"] += torch.sum(torch.abs(tensor_gt - tensor_predict))
        ...
        values["voxel_differ_avg (HU)"] = values["voxel_differ_avg (HU)"] / values["total_locations"] / x / y / z * 1600
...
def training(params):
    ...
    train_dataset = dataset_and_loader.WeightedChestDatasetFocal(
        params["train_data_dir_a"],
        params["train_data_dir_b"],
        mode='train',
        test_id=params["test_id"],
        wrong_file_name=params["wrong_file_name"]
    )

    test_dataset = dataset_and_loader.WeightedChestDatasetFocal(
        params["test_data_dir_a"],
        params["test_data_dir_b"],
        mode='test',
        test_id=params["test_id"],
        wrong_file_name=params["wrong_file_name"]
    )

    print("train:", params["train_data_dir_a"], len(train_dataset))
    print("train:", params["train_data_dir_b"], len(train_dataset))
    print("there are:", len(train_dataset), "training ct scans")
    print("there are:", len(test_dataset), "testing ct scans")

    train_loader = dataset_and_loader.DataLoaderFocal(train_dataset, params["batch_ct"], params["batch_split"],
                                                      ...
                                                      initial_portion=params["initial_portion"],
                                                      end_portion=params["end_portion"],
                                                      ratio_mask_input_end=params["ratio_mask_input_end"],
                                                      ratio_predict_end=params["ratio_predict_end"],
                                                      num_interval=params["num_interval"],
                                                      interval_epoch=params["interval_epoch"])

    test_loader = dataset_and_loader.DataLoaderFocal(test_dataset, params["batch_ct"], params["batch_split"],
                                                     params["ratio_mask_input"], params["ratio_predict"],
