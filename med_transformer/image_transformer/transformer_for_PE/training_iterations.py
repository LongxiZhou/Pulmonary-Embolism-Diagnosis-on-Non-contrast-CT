...
def train_loop(model, optimizer, train_loader, test_loader, params=None):
    ...
    for epoch in range(epoch_start + 1, epoch_start + 1 + params["n_epochs"]):
        ...
        for i, batch_sample in enumerate(train_loader):
            ...
            tensor_mask_depth_greater_4 = torch.FloatTensor(temp_depth_mask_array).cuda()
            ...
            # loss = loss_function.weighted_absolute_difference_loss_tissue_wise(
            #     10 * tensor_predict * tensor_mask_depth_greater_4 + tensor_predict, 10 * tensor_gt * tensor_mask_depth_greater_4 + tensor_gt, tensor_penalty,
            ...
            values["voxel_differ_avg (HU) encoding_depth > 4, info"] += \
                torch.sum(torch.abs((tensor_gt_info - tensor_predict_info) * mask_depth_greater_4_info))
            values["voxel_differ_avg (HU) encoding_depth > 4, query"] += \
                torch.sum(torch.abs((tensor_gt_query - tensor_predict_query) * mask_depth_greater_4_query))

            num_voxel_depth_greater_4_info = torch.sum(tensor_mask_depth_greater_4[:, 0: num_information_cubes, :])
            num_voxel_depth_greater_4_query = torch.sum(tensor_mask_depth_greater_4[:, num_information_cubes::, :])
