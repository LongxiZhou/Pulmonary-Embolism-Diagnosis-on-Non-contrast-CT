...
from classic_models.Unet_3D.U_net_Model_3D_new import UNet3D, UNet3DSimple, UNet3DSimplest
from functools import partial


UNet3D = partial(UNet3D, leaky=0.1, batch_norm=True)
UNet3DSimple = partial(UNet3DSimple, leaky=0.1, batch_norm=True)
UNet3DSimplest = partial(UNet3DSimplest, leaky=0.1, batch_norm=True)
...
class RefineRegistrationFlow(nn.Module):
    ...
    def __init__(self, image_size, num_channel=4, num_landmark=18, depth_get_landmark=2, depth_refine_flow=2,
                 inference_phase=True, split_positive_and_negative=False):
        """
        ...
        self.model_dict = {2: UNet3DSimplest, 3: UNet3DSimple, 4: UNet3D}
        # key is the depth (number of pooling)

        self.inference_phase = inference_phase
        self.split_positive_and_negative = split_positive_and_negative

        self.depth_get_landmark = depth_get_landmark

        if depth_get_landmark == 0:
            self.model_extract_landmark = no_operation
        else:
            # use this model to extract landmark for moving and fixed
            self.model_extract_landmark = \
                self.model_dict[depth_get_landmark](
                    in_channels=num_channel, out_channels=num_landmark, init_features=num_landmark)

        # landmark from moving and fixed will be concat together
        if depth_get_landmark == 0:
            num_landmark_combined = int(num_channel * 2)
        else:
            num_landmark_combined = int(2 * num_landmark) + int(num_channel * 2)
    ...
    def extract_and_combine_landmark(self, pre_registered_image, fixed_image):
        ...
        if self.depth_get_landmark == 0:
            landmark_combined = torch.concat((pre_registered_image, fixed_image), dim=1)
            return landmark_combined
        else:
            ...
            landmark_combined = torch.concat(
                (landmark_pre_registered, landmark_fixed, pre_registered_image, fixed_image), dim=1)
    ...
    def forward(self, moving_image, fixed_image, registration_flow_raw=None):
        ...
        :param moving_image: tensor in shape (N, 4, X, Y, Z)
        :param fixed_image: tensor in shape (N, 4, X, Y, Z)
