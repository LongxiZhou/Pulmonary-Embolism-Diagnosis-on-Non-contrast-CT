import cv2
from classic_models.Unet_2D.test import *
import os
...
if __name__ == '__main__':
    ...
    blood_stage_one = np.load('/home/zhoul0a/Desktop/vein_artery_identification/stage_one_prediction_blood/'
                               + 'f0' + patient + '_blood.npy')
    lungs = np.load('/home/zhoul0a/Desktop/vein_artery_identification/stage_one_prediction_blood/' + 'f0' +
                    patient + '_lung.npz')['array']

    ct = np.load('/home/zhoul0a/Desktop/vein_artery_identification/rescaled_ct/f035_2020-03-10.npy')

    locations = np.where(blood_stage_one > 0.5)
    semantic_center = [int(np.median(locations[0])), int(np.median(locations[1])), int(np.median(locations[2]))]

    gt_all = np.load('/home/zhoul0a/Desktop/vein_artery_identification/rescaled_gt/f031_2020-03-10.npz')['array']
    gt = np.array((gt_all[:, :, :, 3] + gt_all[:, :, :, 4]) > 0.5, 'float32')

    top_dict_sample = '/home/zhoul0a/Desktop/vein_artery_identification/heat_cam_blood'

    data = np.load(top_dict_sample + '/stage_one/bottle_version4/f0' + patient + '_sample_Z.npy')[:, :, 1]
    data = (data - np.min(data)) / np.max(data)

    data = np.clip(data, 0, 0.66) / 0.66

    pic = Functions.merge_image_with_mask(data, gt[:, :, semantic_center[2]], show=True)
    Functions.image_save(pic, '/home/zhoul0a/Desktop/prognosis_project/visualize/supplementary/gt.png', high_resolution=True)

    cam_image = np.load(top_dict_sample + '/stage_one/bottle_version4/f0' + patient + '_heatmap_Z.npy')
    print(np.shape(cam_image))
    cam_image = np.sqrt(cam_image)
    merged_image = merge_with_heat_map(data, cam_image)

    Functions.image_show(merged_image)
    Functions.image_save(merged_image, '/home/zhoul0a/Desktop/prognosis_project/visualize/supplementary/bottle_one.png', high_resolution=True)

    cam_image = np.load(
        top_dict_sample + '/stage_two/bottle_version4/f0' + patient + '_heatmap_Z.npy')
    print(np.shape(cam_image))

    merged_image = merge_with_heat_map(data, cam_image)

    Functions.image_show(merged_image)
    Functions.image_save(merged_image, '/home/zhoul0a/Desktop/prognosis_project/visualize/supplementary/bottle_two.png', high_resolution=True)

    cam_image = np.load(
        top_dict_sample + '/stage_one/last_cnn_version4/f0' + patient + '_heatmap_Z.npy')
    print(np.shape(cam_image))
    cam_image = np.sqrt(cam_image)
    merged_image = merge_with_heat_map(data, cam_image)

    Functions.image_show(merged_image)
    Functions.image_save(merged_image, '/home/zhoul0a/Desktop/prognosis_project/visualize/supplementary/last_one.png', high_resolution=True)

    cam_image = np.load(
        top_dict_sample + '/stage_two/last_cnn_version4/f0' + patient + '_heatmap_Z.npy')
    print(np.shape(cam_image))

    cam_image = np.sqrt(cam_image)

    lung_slice = lungs[:, :, semantic_center[2]]
    locs_array = np.where(lung_slice > 0.5)
    zero = np.zeros([512, 512])
    zero[np.min(locs_array[0]): np.max(locs_array[0]), np.min(locs_array[1]): np.max(locs_array[1])] = 1
    cam_image = cam_image * zero

    merged_image = merge_with_heat_map(data, cam_image)

    Functions.image_show(merged_image)
    Functions.image_save(merged_image, '/home/zhoul0a/Desktop/prognosis_project/visualize/supplementary/last_two.png', high_resolution=True)

    exit()

    from basic_tissue_prediction.three_way_prediction import get_enhance_channel
    ...
    for f in ['bottle']:
        for i in [1, 2, 5, 6, 7, 8, 9]:
            ...
            lungs = np.load('/home/zhoul0a/Desktop/vein_artery_identification/stage_one_prediction_air/' + 'f03' +
                            str(i) + '_lung.npz')['array']

            airway_stage_one = np.load('/home/zhoul0a/Desktop/vein_artery_identification/stage_one_prediction_air/'
                                       + 'f03' + str(i) + '_air.npy')
            gt = np.array(airway_stage_one > 0.5, 'float32')
            model_dict = '/home/zhoul0a/Desktop/prognosis_project/check_points/air_way_seg_stage_one'
            save_dict = '/home/zhoul0a/Desktop/vein_artery_identification/heat_cam_air/stage_one'
            array_info["enhanced_channel"] = 0
            get_cam_map(ct, gt, model_dict, save_dict=save_dict, patient_id='f03' + str(i), feature=f)

            model_dict = '/home/zhoul0a/Desktop/prognosis_project/check_points/air_way_seg_stage_two'
            save_dict = '/home/zhoul0a/Desktop/vein_artery_identification/heat_cam_air/stage_two'
            ...
            enhanced_channel_one, enhanced_channel_two = get_enhance_channel(lungs, airway_stage_one, ratio_low,
                                                                             ratio_high)
            new_input[:, :, :, 1] = enhanced_channel_one
            new_input[:, :, :, 2] = enhanced_channel_two
            array_info["enhanced_channel"] = 2
            get_cam_map(new_input, gt, model_dict, save_dict=save_dict, patient_id='f03' + str(i), feature=f)
    exit()
