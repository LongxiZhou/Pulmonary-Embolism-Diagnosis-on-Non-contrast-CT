...
for y in b:
    x_l_2.append(1)
    y_l_2.append(y)
    x_t_2.append('Scan-level Bias for Baseline')

ax = sns.boxplot(x=x_t + x_t_2, y=y_l + y_l_2, order=['Median Lesion Severity', 'Scan-level Bias for Baseline'])
ax = sns.swarmplot(x=x_t + x_t_2, y=y_l + y_l_2, order=['Median Lesion Severity', 'Scan-level Bias for Baseline'], color=".25")
# ax.set_ylim([0, 50])
