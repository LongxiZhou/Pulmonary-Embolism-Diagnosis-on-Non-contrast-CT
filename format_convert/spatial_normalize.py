...
def rescale_to_new_shape_2d(array, target_shape):
    return cv2.resize(array, (target_shape[1], target_shape[0]), cv2.INTER_LANCZOS4)


def rescale_to_new_shape(array, target_shape, change_format=True):
    # only support 3D rescale
    assert len(target_shape) == len(np.shape(array)) == 3
    original_shape = np.shape(array)

    if change_format:
        if not (array.dtype == 'float32' or array.dtype == 'float64'):
            # print("change data format from", array.dtype, 'to float32')
            array = np.array(array, 'float32')

    if original_shape[0] == target_shape[0] and original_shape[1] == target_shape[1]:
        array_standard_xy = array
    else:
    ...
    if original_shape[2] == target_shape[2]:
        return array_standard_xy
...
def rescale_to_standard(array, resolution, target_resolution=(334/512, 334/512, 1), target_shape=(512, 512, 512),
                        return_final_resolution=False, tissue='lung', change_z_resolution=True, show=False):
    # padding and rescale the array to the same resolution and shape for further processing.
    ...
    if tissue == 'lung':
        assert target_resolution[0: 2] == (334 / 512, 334 / 512)
        ...
        if original_shape[2] * resolution[2] > 450:
            ...
            warnings.warn('The length of the scan is longer than expected.', SyntaxWarning)
            if change_z_resolution:
...
def rescale_to_original_2(original_dcm_dict, rescaled_array, resolution_rescaled=(334/512, 334/512, 1),
    ...
    if original_shape[2] * resolution[2] > 450 and change_z_resolution:
        print('the length of the lung is:', original_shape[2] * resolution[2])
...
if __name__ == '__main__':
    test_array = np.zeros([10, 5], 'float32')
    print(np.shape(rescale_to_new_shape_2d(test_array, (7, 6))))
    exit()
