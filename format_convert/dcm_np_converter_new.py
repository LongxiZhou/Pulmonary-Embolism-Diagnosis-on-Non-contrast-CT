...
def get_resolution_from_dcm(dcm_dict, show=True):
    ...
    for index in range(1, len(slice_id_z_location_list)):
        ...
        if not interval_slices > 0 and warn:
            warnings.warn(
                '\n##############################\nDetected multiple dcm files at the same location!\n'
                'May be different scans or protocol into same file, please re-check to guarantee performances'
                '\n##############################\n')
            continue
