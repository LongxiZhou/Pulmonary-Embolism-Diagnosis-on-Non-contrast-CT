...
def get_branch_for_center_line(center_line_mask, depth_array, radius=5, radius_smooth=1, interval=1):
    ...
    loc_list_center_line = Functions.get_location_list(np.where(center_line_mask > 0.5))
...
if __name__ == '__main__':
    ...
    fn_list = os.listdir('/data_disk/RAD-ChestCT_dataset/blood_depth_and_center-line/depth_mask/')
    ...
    blood_center_line = \
    np.load('/data_disk/RAD-ChestCT_dataset/blood_depth_and_center-line/blood_center_line/' + fn_name)['array']
    blood_depth_mask = np.load('/data_disk/RAD-ChestCT_dataset/blood_depth_and_center-line/depth_mask/' + fn_name)[
        'array']
    ...
    center_line_with_depth = get_branch_for_center_line(blood_center_line, blood_depth_mask, radius_smooth=2)

    for i in range(250, 300, 1):
        image_left = center_line_with_depth[:, :, i]

        max_depth = np.max(center_line_with_depth)

        image_right = surface[:, :, i] * max_depth
    ...
    test_center_line_depth = np.zeros([200, 200, 200], 'float16')

    test_center_line_depth[(100, 120, 130)] = 1.6
    test_center_line_depth[(150, 140, 90)] = 3.6

    test_vessel_mask = np.zeros([200, 200, 200], 'float16')

    test_vessel_mask[30:180, 100: 160, 60: 190] = 1

    test_point_cloud = propagate_branching_cloud(test_center_line_depth, test_vessel_mask)

    Functions.show_point_cloud_3d(test_point_cloud)

    for i in range(125, 135):
        Functions.image_show(test_point_cloud[:, :, i])
