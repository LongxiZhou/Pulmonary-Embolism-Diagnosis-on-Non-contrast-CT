...
clot_sample_dict["clot_depth_dict"] = {(x, y, z): b, ..., 'max_depth': max_clot_depth}  here b is the clot depth
...
def get_clot_sample_dict_from_loc_list(loc_list_clot, remove_hole=True):
    ...
    clot_sample_dict["clot_depth_dict"]["max_depth"] = np.max(depth_array)
...
def form_sample_dict_list_from_ct(ct_data, lung_mask, save_path, strict=False, change_to_hu=True):
    """

    :param change_to_hu:
    :param ct_data:
    :param lung_mask:
    :param save_path: /./file_name.pickle
    :param strict:
    :return:
    """
    import time
    if change_to_hu:
        ct_data = Functions.change_to_HU(ct_data) / 5
    if lung_mask is None:
        bounding_lung = [[0, np.shape(ct_data)[0]], [0, np.shape(ct_data)[1]], [0, np.shape(ct_data)[2]]]
    else:
        bounding_lung = Functions.get_bounding_box(lung_mask)
        ct_data = ct_data * lung_mask

    ct_data = np.array(ct_data, 'int16')
    # have to cast to int for "connect_region_detect.get_connected_regions_discrete"

    sub_array = ct_data[bounding_lung[0][0]: bounding_lung[0][1], bounding_lung[1][0]: bounding_lung[1][1],
...
if __name__ == '__main__':
    import os
    save_top_dict = '/home/zhoul0a/Desktop/pulmonary_embolism/clot_simulation/simulate_based_on_original_ct/'

    fn_list = os.listdir('/data_disk/rescaled_ct_and_semantics/rescaled_ct/healthy_people/xwzc/')

    for fn in fn_list[0: 3]:
        test_array = np.load(
            '/data_disk/rescaled_ct_and_semantics/rescaled_ct/healthy_people/xwzc/' + fn)[
            'array']
