...
class U_Network_down(nn.Module):
    def __init__(self, dim, enc_nf, dec_nf, bn=None, full_size=True):
        super(U_Network_down, self).__init__()
        self.bn = bn
        self.dim = dim
        self.enc_nf = enc_nf
        self.full_size = full_size
...
class VxmDense(nn.Module):
    ...
    def forward(self, src, tgt, src_seg, tgt_seg):
        ...
        flow_field = F.interpolate(flow_field * self.scale, scale_factor=self.scale)
        ...
        return (src_re, src_seg_re, tgt_re, tgt_seg_re, pos_flow) if self.bidir else (src_re, src_seg_re, pos_flow)
