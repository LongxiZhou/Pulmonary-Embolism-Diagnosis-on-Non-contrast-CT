...
def train():
    ...
    model_0.load_state_dict(
        torch.load("/home/chuy/Train_and_Test/registration/lung_register/128_model/model_epoch_1260.pth"))
    ...
    model_1.load_state_dict(
        torch.load("/home/chuy/Train_and_Test/registration/lung_register/256_model/model_epoch_26.pth"))

    # print("===> Building model_1")
    # scale_factor = 1
    # vol_size = [384, 512, 320]
    # nf_enc = [1, 2, 4, 8]
    # nf_dec = [8, 4, 4, 2, 2, 1, 1]
    # model_2 = Register_1(inshape=vol_size, unet_encoder=nf_enc, unet_decoder=nf_dec, scale=scale_factor, use_morph=False)
    # print("===> Loading datasets")
    ...
    for epoch in range(1, opt.nEpochs + 1):
        print(epoch)
        trainor(optimizer, model_0, model_1, training_data_loader_1, training_data_loader_2, epoch, scheduler)


def trainor(optimizer, model_0, model_1, loader_1, loader_2, epoch, scheduler):
    # print("===> Loading datasets")
    ...
    for iteration, (fixed, moving) in enumerate(zip(loader_1, loader_2)):
        ...
        raw_img = fixed_img.cpu().detach().numpy()[0, 0]
        plt.imshow(raw_img[:, :, 128], cmap="gray")
        plt.show()

        registered_img = registered_img_1.cpu().detach().numpy()[0, 0]
        plt.imshow(registered_img[:, :, 128], cmap="gray")
        plt.show()
        # (registered_img_2, registered_seg_2, pos_flow) = \
        #     model_2(registered_img_1, fixed_img, registered_seg_1, fixed_seg)

        # dice_loss = dice_loss_fn(registered_seg_1, fixed_seg)
        ...
        # pos_flow = F.interpolate(pos_flow, scale_factor=0.5)
        # grad_loss = grad_loss_fn(pos_flow)
        #
        # loss = dice_loss + 5 * (mean_loss + grad_loss)
        ...
        print("===> Epoch[{}]: DiceLoss1: {:.5f} DiceLoss2: {:.5f} Loss: {:.5f} loss_avg: {:.5f}".format
              (epoch, dice_loss_1, dice_loss_2, grad_loss, loss_epoch / (iteration % 100 + 1)))

        if (iteration + 1) % 100 == 0:
            loss_epoch = 0
            save_checkpoint(model_1, epoch, "/home/chuy/Train_and_Test/registration/lung_register/256_model/")
