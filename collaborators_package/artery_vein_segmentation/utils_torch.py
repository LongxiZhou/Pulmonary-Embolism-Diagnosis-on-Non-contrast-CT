...
def save_checkpoint(state, is_best, checkpoint_dir, logger=None):
    """Saves model_guided and training parameters at '{checkpoint_dir}/last_checkpoint.pytorch'.
    If is_best==True saves '{checkpoint_dir}/best_checkpoint.pytorch' as well.

    Args:
        state (dict): contains model_guided's state_dict, optimizer's state_dict, epoch
            and best evaluation metric value so far
        is_best (bool): if True state contains the best model_guided seen so far
...
def load_checkpoint(checkpoint_path, model, optimizer=None):
    """Loads model_guided and training parameters from a given checkpoint_path
    If optimizer is provided, loads optimizer's state_dict of as well.

    Args:
        checkpoint_path (string): path to the checkpoint to be loaded
        model (torch.nn.Module): model_guided into which the parameters are to be copied
...
class _TensorboardFormatter:
    ...
    image) to a series of images that can be displayed in tensorboard. This is the parent class for all_file tensorboard
...
class EmbeddingsTensorboardFormatter(DefaultTensorboardFormatter):
    ...
    def _pca_project(self, embeddings):
        ...
        # fit the model_guided with embeddings and apply the dimensionality reduction
