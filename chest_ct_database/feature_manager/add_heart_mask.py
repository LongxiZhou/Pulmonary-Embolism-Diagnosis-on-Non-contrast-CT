...
def smooth_mask(heart_mask, surface_add=1):
    if surface_add == 0:
        return heart_mask
    for surface in range(surface_add):
        heart_mask = heart_mask + get_surface.get_surface(heart_mask, outer=True, strict=False)
    heart_mask = heart_mask - get_surface.get_surface(heart_mask, outer=False, strict=False)
    return heart_mask


def predict_heart_masks_rescaled_array(rescaled_array, batch_size=2, refine=True,
    ...
    :param refine: whether refine heart mask. refine heart will take about 10 secs each scan.
    :return: heart mask for the rescaled array, binary numpy array in shape [512, 512, 512], 0 outer heart 1 inner heart.
    """

    print("predicting heart masks\n")
    heart_mask = three_way_prediction.three_way_predict_binary_class_faster(
        rescaled_array, None, threshold, batch_size, loaded_models, array_info)
    if not refine:
        return heart_mask
    else:
        heart_mask = connectivity_refine.refine_mask(heart_mask, None, 1, lowest_ratio=0.1)
        return heart_mask
...
def func_file_operation(list_top_dict_reference, dataset_sub_dir, file_name, model=None, batch_size=2, array_info=None,
    ...
    heart_mask = predict_heart_masks_rescaled_array(rescaled_ct, batch_size=batch_size, refine=refine,
                                                    loaded_models=model,
                                                    array_info=array_info)

    heart_mask = smooth_mask(heart_mask, smooth_surface)

    return heart_mask


def func_file_save(save_dict, file_name, feature_package):
    Functions.save_np_array(os.path.join(save_dict, 'heart_mask'), file_name[:-4] + '.npz', feature_package,
                            compress=True, dtype='float16')


def func_check_processed(save_dict, file_name):
    path_saved = os.path.join(save_dict, 'heart_mask', file_name[:-4] + '.npz')
    if os.path.exists(path_saved):
        return True
    return False


def add_heart_seg(top_dict_rescaled_ct, top_dict_save, fold=(0, 1)):
    array_info = {
        "resolution": (1, 1, 1),
        "data_channel": 1,
        "enhanced_channel": 0,
    ...
    check_point_dict = '/home/zhoul0a/Desktop/prognosis_project/check_points/heart_seg'
...
if __name__ == '__main__':
    ...
    add_heart_seg('/data_disk/RSNA-PE_dataset/simulated_non_contrast/rescaled_ct-denoise/',
