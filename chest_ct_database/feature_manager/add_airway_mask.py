...
from basic_tissue_prediction.predict_rescaled import predict_airway_stage_one_rescaled_array, get_bounding_box
...
def get_prediction_airway(rescaled_array, stage_one_array=None, lung_mask=None, check_point_top_dict=None,
                          batch_size=64, probability_only=False, refine_airway=True,
    ...
    return stage two probability mask and airways vessel, airway_depth
    ...
    :param refine_airway: whether use connectivity refine on airways vessels, take about 30 seconds each
    ...
    if stage_one_array is None:
        stage_one_array = predict_airway_stage_one_rescaled_array(
    ...
    if refine_airway:
        prediction = connectivity_refine.refine_mask(prediction, None, 7, lowest_ratio=0.01)
...
def func_file_operation(list_top_dict_reference, dataset_sub_dir, file_name, batch_size=2,
    ...
    vessel_mask = get_prediction_airway(rescaled_ct, None, lung_mask, None, batch_size, False,
                                        refine)
...
def func_file_save(save_dict, file_name, feature_package):
    Functions.save_np_array(os.path.join(save_dict, 'airway_mask'), file_name[:-4] + '.npz', feature_package,
                            compress=True, dtype='float16')


def func_check_processed(save_dict, file_name):
    path_saved = os.path.join(save_dict, 'airway_mask', file_name[:-4] + '.npz')
    if os.path.exists(path_saved):
        return True
    return False


def add_airway_seg(top_dict_rescaled_ct, top_dict_save, fold=(0, 1)):
...
if __name__ == '__main__':
    ...
    add_airway_seg('/data_disk/RSNA-PE_dataset/simulated_non_contrast/rescaled_ct-denoise/',
