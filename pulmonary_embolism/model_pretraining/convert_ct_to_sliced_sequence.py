    ...
    for x in range(x_min, x_max, step[0]):
        for y in range(y_min, y_max, step[1]):
            for z in range(z_min, z_max, step[2]):
                ...
                sample = extract_cube(rescaled_ct, penalty_array, cube_length, central_location,
                                      central_location_offset, target_shape, depth_array=None)
...
def pipeline_extract_mask(rescaled_ct, penalty_array, cube_length, semantic_mask, mass_center, shift=(0, 0, 0),
    ...
    for x in range(x_min, x_max, step[0]):
        for y in range(y_min, y_max, step[1]):
            for z in range(z_min, z_max, step[2]):
                if semantic_mask[x, y, z] < 0.5:  # the central_location outside the lung mask
                    continue
                if num_sample > max_cube_count:
                    break
                central_location = (x, y, z)  # central_location for the cube
...
def extract_cube(rescaled_ct, penalty_array, cube_length, central_location, central_location_offset,
    ...
    assert np.shape(penalty_array) == (np.shape(rescaled_ct)[0], np.shape(rescaled_ct)[1], np.shape(rescaled_ct)[2], 4)
    cube_radius_x = int(cube_length[0] / 2)
    cube_radius_y = int(cube_length[1] / 2)
    cube_radius_z = int(cube_length[2] / 2)
    ...
    penalty_cube = penalty_array[central_location[0] - cube_radius_x: central_location[0] + cube_radius_x + 1,
                                 central_location[1] - cube_radius_y: central_location[1] + cube_radius_y + 1,
                                 central_location[2] - cube_radius_z: central_location[2] + cube_radius_z + 1, :]
    ...
    return_dict = {'ct_data': ct_cube, 'penalty_weight': penalty_cube, 'location_offset': central_location_offset,
...
def get_penalty_array_2(rescaled_ct, vessel_mask, airway_mask, lung_mask, heart_mask, show=False):
    """
    ...
    lung_heart_mask = np.clip(lung_mask + heart_mask, 0, 1)

    # penalty sum for each semantic is: total_penalty / std_semantic * std_overall
    penalty_array[:, :, :, 3] = base_penalty
    penalty_array[:, :, :, 2] = \
        base_penalty * std_overall / std_lung_mask * total_voxel / np.sum(lung_mask) * lung_heart_mask
    penalty_array[:, :, :, 1] = \
        base_penalty * std_overall / std_airway * total_voxel / np.sum(airway_mask) * airway_mask
    penalty_array[:, :, :, 0] = \
        base_penalty * std_overall / std_vessel * total_voxel / np.sum(vessel_mask) * vessel_mask
...
def pipeline_process_2():
    ...
    for file_name in list_file_name:
        ...
        if os.path.exists(top_dict_extract_count_mask + 'lung_heart/' + file_name[:-4] + '.npz'):
            print('processed')
            processed_count += 1
            continue
        ...
        penalty_array = get_penalty_array_2(rescaled_ct, vessel_mask, airway_mask, lung_mask, heart_mask, show=True)

        heart_mask = heart_mask + get_surface.get_surface(heart_mask, outer=True, strict=False)
        heart_mask = heart_mask + get_surface.get_surface(heart_mask, outer=True, strict=False)

        vessel_extended = blood_robust_mask + get_surface.get_surface(blood_robust_mask, outer=True, strict=False)
        vessel_extended = vessel_extended + get_surface.get_surface(vessel_extended, True, False)

        for i in range(20):
            vessel_extended = vessel_extended + get_surface.get_surface(vessel_extended, True, False)

        lung_heart_mask = np.clip(lung_mask + heart_mask + vessel_extended, 0, 1)
        lung_heart_mask = lung_heart_mask + get_surface.get_surface(lung_heart_mask, outer=True, strict=False)

        list_sample_lung_heart, extract_count_mask_lung_heart = convert_ct_into_tubes(
            rescaled_ct, vessel_mask, airway_mask, lung_heart_mask, mode="lung", penalty_array=penalty_array)
        print("num_lung_heart_cubes:", len(list_sample_lung_heart))
        save_dict_pickle_lung_heart = top_dict_dataset + 'lung_heart/'
        save_dict_check_array_lung_heart = top_dict_extract_count_mask + 'lung_heart/'
        Functions.pickle_save_object(save_dict_pickle_lung_heart + file_name[:-4] + '.pickle', list_sample_lung_heart)
        Functions.save_np_array(save_dict_check_array_lung_heart, file_name[:-4] + '.npz',
                                extract_count_mask_lung_heart, True)

        processed_count += 1


def pipeline_process_3(top_dict_dataset=None, top_dict_image_check=None, top_dict_rescaled_ct=None,
                       top_dict_semantic=None, path_dict_report=None):
    # generate dataset for COVID-19 patient, use the "dict_report" to save the statistics for further exclusion.
    import visualization.visualize_3d.highlight_semantics as highlight
    if top_dict_dataset is None:
        top_dict_dataset = '/home/zhoul0a/Desktop/pulmonary_embolism/dataset_normal_v2/list_pickle_dataset_mudanjiang/'
    if top_dict_image_check is None:
        top_dict_image_check = \
            '/home/zhoul0a/Desktop/pulmonary_embolism/dataset_normal_v2/list_pickle_dataset_mudanjiang/image_check/'
    if top_dict_rescaled_ct is None:
        top_dict_rescaled_ct = '/media/zhoul0a/New Volume/rescaled_ct_and_semantics/rescaled_ct/COVID-19/mudanjiang/'
    if top_dict_semantic is None:
        top_dict_semantic = '/media/zhoul0a/New Volume/rescaled_ct_and_semantics/semantics/mudanjiang/'
    if path_dict_report is None:
        path_dict_report = \
            '/home/zhoul0a/Desktop/pulmonary_embolism/dataset_normal_v2/list_pickle_dataset_mudanjiang/dict_report' \
            '.pickle '

    print("top dict dataset:", top_dict_dataset)
    print("top_dict_image_check:", top_dict_image_check)
    print("top_dict_for_file_names:", top_dict_rescaled_ct)
    print("top_dict_semantic:", top_dict_semantic)
    print("path_dict_report:", path_dict_report)

    list_file_name = os.listdir(top_dict_rescaled_ct)

    if os.path.exists(path_dict_report):
        dict_report = Functions.pickle_load_object(path_dict_report)  # key is the file name for .npy files
    else:
        dict_report = {}
    ...
    for file_name in list_file_name:
        ...
        if file_name in list(dict_report.keys()):
            print('processed')
            processed_count += 1
            continue
        ...
        heart_mask = np.load(top_dict_semantic + 'heart_mask/' + file_name[:-4] + '.npz')['array']
        rescaled_ct = np.load(top_dict_rescaled_ct + file_name)
        lung_mask = np.load(top_dict_semantic + 'lung_mask/' + file_name[:-4] + '.npz')['array']
        vessel_mask = np.load(top_dict_semantic + 'blood_mask/' + file_name[:-4] + '.npz')['array']

        rescaled_ct_clip = np.clip(np.array(rescaled_ct, 'float32') + 0.5, 0, 1.1) / 1.1
        highlighted = highlight.highlight_mask(lung_mask, rescaled_ct_clip, 'G', False, 0.2)
        highlighted = highlight.highlight_mask(vessel_mask, highlighted, 'R', True, 0.2)
        highlighted = highlight.highlight_mask(airway_mask, highlighted, 'B', True, 0)

        image = highlighted[:, :, 256]
        Functions.image_save(image, top_dict_image_check + file_name[:-4] + '.png', dpi=300)

        penalty_array = get_penalty_array_2(rescaled_ct, vessel_mask, airway_mask, lung_mask, heart_mask, show=True)

        # generate blood vessel samples
        list_sample_blood, extract_count_mask_blood = convert_ct_into_tubes(rescaled_ct, vessel_mask, airway_mask,
                                                                            lung_mask, mode="blood_vessel",
                                                                            penalty_array=penalty_array)
        print("num_blood_vessel_cubes:", len(list_sample_blood))
        ...
        # get extended lung heart samples
        heart_mask = heart_mask + get_surface.get_surface(heart_mask, outer=True, strict=False)
        heart_mask = heart_mask + get_surface.get_surface(heart_mask, outer=True, strict=False)

        vessel_extended = vessel_mask + get_surface.get_surface(vessel_mask, outer=True, strict=False)
        vessel_extended = vessel_extended + get_surface.get_surface(vessel_extended, True, False)
        ...
        blood_vessel_center_line_voxel = np.sum(get_center_line.get_center_line(vessel_mask))
        ...
        case_dict = {'lung_volume': lung_volume, 'lung_surface_area': lung_surface_area, 'heart_volume': heart_volume,
                     ...
                     'blood_vessel_volume': blood_vessel_volume, 'blood_vessel_surface_area': blood_vessel_surface_area,
                     'infection_volume': infection_volume, 'infection_to_lung_ratio': infection_to_lung_ratio}

        print("case_dict")
        print(case_dict)

        dict_report[file_name] = case_dict

        Functions.pickle_save_object(path_dict_report, dict_report)
