...
def func_exclusion_rad_dataset(semantic_report, sequence_name):
    ...
    if case_dict['infection_to_lung_ratio'] > 0.075:
        print(sequence_name, 'bad sample: infection', case_dict['infection_to_lung_ratio'])
        return True
    if case_dict['blood_vessel_center_line_voxel'] < 6000:
        print(sequence_name, 'bad sample: blood center line', case_dict['blood_vessel_center_line_voxel'])
        return True
    ...
    if max(blood_value_list) < 10 or min(blood_value_list) > 100:
        print(sequence_name, 'bad sample: blood average signal', blood_value_list)
        return True
...
if __name__ == '__main__':
    ...
    report_mudanjiang = Functions.pickle_load_object('/data_disk/rescaled_ct_and_semantics/reports/COVID-19/mudanjiang/report_dict.pickle')
    report_yidayi = Functions.pickle_load_object('/data_disk/rescaled_ct_and_semantics/reports/COVID-19/yidayi/report_dict.pickle')
    report_four_center = Functions.pickle_load_object('/data_disk/rescaled_ct_and_semantics/reports/healthy_people/four_center_data/report_dict.pickle')
    report_xwzc = Functions.pickle_load_object('/data_disk/rescaled_ct_and_semantics/reports/healthy_people/xwzc/report_dict.pickle')
    report_refine = Functions.pickle_load_object('/home/zhoul0a/Desktop/pulmonary_embolism/refine_dataset/report_dict.pickle')

    report_merged = merge_dicts([report_rad, report_mudanjiang, report_yidayi, report_four_center, report_xwzc, report_refine])
    print(len(report_merged))

    sequence_dataset_dict = '/home/zhoul0a/Desktop/pulmonary_embolism/vessel_dataset_high_resolution_denoise_new/merged_3000_vessel_only'
