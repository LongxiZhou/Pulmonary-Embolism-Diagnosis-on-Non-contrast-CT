...
def get_prediction_invisible_lesion_covid_19(rescaled_array, check_point_top_dict=None, lung_mask=None, airway=None,
                                             blood_vessel=None, threshold=2., batch_size=64, general_sampling=True, save_enhance_path=None):
...
if __name__ == '__main__':
    ...
    enhanced = np.load('/home/zhoul0a/Desktop/prognosis_project/original_follow_up/parenchyma_enhanced_arrays/xghf-25_2020-07-29.npy')

    invisible = get_invisible_covid_19_lesion_from_enhanced(enhanced)

    enhance_slice = np.clip(enhanced[:, :, 308], 0, 0.1)

    Functions.merge_image_with_mask(enhance_slice, invisible[:, :, 308])
