...
def predict_lung_masks_rescaled_array(rescaled_array, check_point_top_dict=None, batch_size=4, refine=False,
                                      threshold=2.):
    """
    :param threshold:
...
def predict_heart_rescaled_array(rescaled_array, check_point_top_dict=None, threshold=1.8, batch_size=64, refine=False):
    """
    :param threshold:
    :param rescaled_array: numpy array in shape [512, 512, 512]
    :param check_point_top_dict: where the model_guided saved, should in check_point_top_dict/semantic/direction/model_name.pth
...
def predict_blood_vessel_stage_one_rescaled_array(rescaled_array, check_point_top_dict=None, batch_size=8):
    global array_info
    original_array_info = array_info
    if check_point_top_dict is None:
        check_point_top_dict = top_directory_check_point
...
def get_prediction_blood_vessel(rescaled_array, stage_one_array=None, lung_mask=None, check_point_top_dict=None,
                                batch_size=4, probability_only=False, refine_lung=False, refine_blood_vessel=True,
...
def surrounding_mean_convolution(probability_mask):
    ...
    if len(shape) == 3:
        ...
        raise ValueError("shape of array have to with dimension 3 or 4")
