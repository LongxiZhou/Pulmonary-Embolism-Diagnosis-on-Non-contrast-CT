...
def refine_masks_all_one_thread_2(inputs):
    ...
    for mask_name in mask_name_list:
        if lung_mask_dict is not None:  # use lung mask means further refined = refined * vessel_mask
...
def refine_mask(mask, restrictive_mask=None, connected_num=7, lowest_ratio=0.001, lowest_voxel=0):
    """
    :param lowest_voxel: neglect if the component with less points than lowest_voxel
    ...
    :param restrictive_mask: if not None, means we use the mask = restrictive_mask * vessel_mask to pre-refine data
    ...
    print("originally there are:", original_volume, 'positive points')
    ...
    while key < connected_num + 1:
        locations = id_loc_dict[key]
        if len(locations) < length_max * lowest_ratio or len(locations) < lowest_voxel:
            print("too small components, finally use:", key - 1, "num of components")
            break
        for loc in locations:
            refined_mask[loc] = 1
