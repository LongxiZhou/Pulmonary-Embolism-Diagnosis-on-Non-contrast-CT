...
def get_input_output_and_penalty_array(rescaled_ct_denoise, blood_branch_map, case_report=None,
                                       blood_vessel_mask=None, lesion_loc_array=None, apply_cta_threshold=False,
                                       blood_center_line_mask=None, version='0'):
    """
    ...
    :param apply_cta_threshold:
    :param lesion_loc_array:
    :param rescaled_ct_denoise:
    :param blood_branch_map:
    :param case_report:
    ...
    if version == '2':
        assert apply_cta_threshold

    if version == '1' or version == '0':
        assert not apply_cta_threshold

    blood_branch_map = np.clip(blood_branch_map, 0, 10)
    if blood_vessel_mask is not None:
        raw_mask = np.array(blood_vessel_mask > 0, 'float32')
        raw_mask_from_branch = np.array(blood_branch_map > 0, 'float32') + np.array(blood_branch_map < 0, 'float32')
        missing_branch_region = np.clip(raw_mask - raw_mask_from_branch, 0, 1)
        # here the missing part give zero fn penalty
        blood_branch_map = blood_branch_map + missing_branch_region * np.max(blood_branch_map)
    else:
        raw_mask = np.array(blood_branch_map > 0, 'float32') + np.array(blood_branch_map < 0, 'float32')

    raw_mask = connect_region_detect.convert_to_simply_connected(raw_mask, max_hole_radius=1)

    if apply_cta_threshold:
        blood_region = get_blood_region(rescaled_ct_denoise, raw_mask, case_report)
    else:
        blood_region = np.array(raw_mask * rescaled_ct_denoise > Functions.change_to_rescaled(-100), 'float32')

    model_output_gt = raw_mask - blood_region

    location_array_gt = np.where(model_output_gt > 0.5)

    branch_value_gt = blood_branch_map[location_array_gt]
    penalty_value = Functions.cast_to_0_1(np.max(branch_value_gt) - branch_value_gt)

    penalty_fn = np.zeros(np.shape(raw_mask), 'float32')
    total_penalty_fn = 100 * np.sqrt(len(location_array_gt[0]))
    penalty_value = penalty_value * (total_penalty_fn / np.sum(penalty_value))
    penalty_fn[location_array_gt] = penalty_value
    average_penalty_fn = total_penalty_fn / len(location_array_gt[0])
    print("total gt voxel:", len(location_array_gt[0]))
    print("total penalty fn:", total_penalty_fn, "average penalty fn:", average_penalty_fn)

    if lesion_loc_array is None:
        return_list = [raw_mask, model_output_gt, penalty_fn, blood_region]
    else:
        raw_mask_with_lesion, model_output_gt_with_lesion, penalty_fn_with_lesion = apply_lesion(
            raw_mask, model_output_gt, penalty_fn, blood_region, lesion_loc_array, random_placement=True)
        return_list = [raw_mask_with_lesion, model_output_gt_with_lesion, penalty_fn_with_lesion, blood_region]

    # The following calculates the penalty for fp

    # model only do reduction, high penalty is given for background regions
    if version == '0':
        penalty_fp = average_penalty_fn - average_penalty_fn * return_list[0]
    elif version == '1' or version == '2':
        penalty_fp = average_penalty_fn * 0.1 - average_penalty_fn * 0.1 * return_list[0]
    else:
        raise ValueError
    # we encourage the model to remove some of the surface, so only give low average_penalty_fn to blood region
    penalty_fp = penalty_fp + blood_region * (total_penalty_fn / np.sum(blood_region))
    # but for deeper region we give high penalty
    # center line give high penalty
    ...
    if version == '0':
        penalty_fp = penalty_fp + blood_region * blood_center_line_mask * (
                total_penalty_fn * 0.25 / np.sum(blood_center_line_mask))
    elif version == '1' or version == '2':
        penalty_fp = penalty_fp + blood_region * blood_center_line_mask * (
                total_penalty_fn * 0.5 / np.sum(blood_center_line_mask))
    else:
        raise ValueError
    # deeper surface give high penalty
    surface_layer_1 = get_surface.get_surface(blood_region, outer=False, strict=False)
    surface_layer_2 = get_surface.get_surface(blood_region - surface_layer_1, outer=False, strict=False)
    if version == '0':
        penalty_fp = penalty_fp + surface_layer_2 * (total_penalty_fn * 0.75 / np.sum(surface_layer_2))
    elif version == '1' or version == '2':
        penalty_fp = penalty_fp + surface_layer_1 * (total_penalty_fn * 0.5 / np.sum(surface_layer_1))
        penalty_fp = penalty_fp + surface_layer_2 * (total_penalty_fn / np.sum(surface_layer_2))
    else:
        raise ValueError
...
def apply_lesion(raw_mask, model_output_gt, penalty_fn, blood_region, lesion_loc_array, random_placement=True,
                 show=False, lesion_penalty=0.5):
    """

    :param show:
    :param blood_region:
    :param raw_mask: the mask for blood vessel (input for model)
    :param model_output_gt: raw_mask - blood_region (output for model)
    :param penalty_fn: the penalty for false negative
    :param lesion_loc_array: return of np.where(), like (array_loc_x, array_loc_y, array_loc_z)
    :param random_placement: True for randomly place the lesion on the vessel, otherwise lesion_loc_array gave precise
    location for lesions
    :param lesion_penalty: fn penalty for lesion voxel, the average fn penalty is from 0.15 to 0.5 for different scans
    :return: raw_mask_with_lesion, model_output_gt (with lesion), penalty_fn (with_lesion)
    """
    ...
    if not random_placement:
        lesion_loc_aligned = lesion_loc_array
        center_loc_lesion = origin_lesion_mass_center
    else:
        loc_array_surface = np.where(model_output_gt > 0)
        pixel_id = random.randint(0, len(loc_array_surface[0]) - 1)
        # pick a random location
        center_loc_lesion = (loc_array_surface[0][pixel_id], loc_array_surface[1][pixel_id],
                             loc_array_surface[2][pixel_id])
    ...
    # trim the lesion loc
    lesion_loc_aligned = (np.clip(lesion_loc_aligned[0], 0, shape_mask[0] - 1),
                          np.clip(lesion_loc_aligned[1], 0, shape_mask[1] - 1),
                          np.clip(lesion_loc_aligned[2], 0, shape_mask[2] - 1))

    raw_mask_with_lesion[lesion_loc_aligned] = 1  # add lesion on it

    model_output_gt_with_lesion = raw_mask_with_lesion - blood_region
    penalty_fn_with_lesion = np.array(penalty_fn, 'float32')  # 0 - 1 array
    penalty_fn_with_lesion[lesion_loc_aligned] = lesion_penalty  # assign high penalty for lesion
    penalty_fn_with_lesion = penalty_fn_with_lesion * model_output_gt_with_lesion

    if show:
        Functions.merge_image_with_mask(raw_mask_with_lesion[:, :, center_loc_lesion[2]],
                                        model_output_gt_with_lesion[:, :, center_loc_lesion[2]])
        Functions.image_show(penalty_fn_with_lesion[:, :, center_loc_lesion[2]])

    return raw_mask_with_lesion, model_output_gt_with_lesion, penalty_fn_with_lesion
...
def get_stack_array_128(semantic='artery', fold=(0, 1)):
    """

    form the stack array for each case (no lesion):
    array with shape [4, 128, 128, 128], each channel is: raw_mask, model_output_gt, penalty_fn, penalty fp
    ...
    if cta:
        ...
        save_top_dict = '/data_disk/artery_vein_project/extract_blood_region/training_data/complete_array/CTA/'
    else:
        data_top_dict = '/data_disk/artery_vein_project/new_data/non-contrast/'
        save_top_dict = \
            '/data_disk/artery_vein_project/extract_blood_region/training_data/complete_array/non-contrast/'

    assert semantic in ['artery', 'vein']
    if semantic == 'artery':
        save_dict = save_top_dict + 'stack_array_artery/'
        branch_dict = data_top_dict + 'depth_and_center-line/artery_branch_map/'
        vessel_mask_dict = data_top_dict + 'semantics/artery_mask/'
        depth_dict = data_top_dict + 'depth_and_center-line/depth_array_artery/'
    else:
        save_dict = save_top_dict + 'stack_array_vein/'
        branch_dict = data_top_dict + 'depth_and_center-line/vein_branch_map/'
        vessel_mask_dict = data_top_dict + 'semantics/vein_mask/'
        depth_dict = data_top_dict + 'depth_and_center-line/depth_array_vein/'

    top_dict_rescaled_ct = data_top_dict + 'rescaled_ct-denoise/'
    fn_list = os.listdir(top_dict_rescaled_ct)[fold[0]::fold[1]]

    report = Functions.pickle_load_object(data_top_dict + 'reports/'
                                          'report_dict.pickle')
    ...
    for fn in fn_list:
        ...
        if os.path.exists(os.path.join(save_dict, fn)):
            ...
            count += 1
            continue
        rescaled_ct_denoise = np.load(top_dict_rescaled_ct + fn)['array']
        case_report = report[fn[:-4]]
        blood_branch_map = np.load(branch_dict + fn)['array']
        blood_vessel_mask = np.load(vessel_mask_dict + fn)['array']

        depth_array = np.load(depth_dict + fn)['array']
        print("average blood HU:",
              np.average(rescaled_ct_denoise[np.where(depth_array > 0.5 * np.max(depth_array))]) * 1600 - 600)

        raw_mask, model_output_gt, penalty_fn, _, penalty_fp = get_input_output_and_penalty_array(
            rescaled_ct_denoise, blood_branch_map, case_report, blood_vessel_mask, None)

        bounding_box = Functions.get_bounding_box(raw_mask, pad=0)

        x_min = bounding_box[0][0]
        x_max = bounding_box[0][1]
        if (x_max - x_min) % 128 > 0:
            total_pad_distance = 128 - ((x_max - x_min) % 128)
            x_min = x_min - int(total_pad_distance / 2)
            x_max = x_max + total_pad_distance - int(total_pad_distance / 2)

        y_min = bounding_box[1][0]
        y_max = bounding_box[1][1]
        if (y_max - y_min) % 128 > 0:
            total_pad_distance = 128 - ((y_max - y_min) % 128)
            y_min = y_min - int(total_pad_distance / 2)
            y_max = y_max + total_pad_distance - int(total_pad_distance / 2)

        z_min = bounding_box[2][0]
        z_max = bounding_box[2][1]
        if (z_max - z_min) % 128 > 0:
            total_pad_distance = 128 - ((z_max - z_min) % 128)
            z_min = z_min - int(total_pad_distance / 2)
            z_max = z_max + total_pad_distance - int(total_pad_distance / 2)

        if x_min < 0 or x_max > 512:
            x_min, x_max = 0, 512
        if y_min < 0 or y_max > 512:
            y_min, y_max = 0, 512
        if z_min < 0 or z_max > 512:
            z_min, z_max = 0, 512

        print("bounding size:", ((x_min, x_max), (y_min, y_max), (z_min, z_max)))

        stack_array = np.zeros([4, x_max - x_min, y_max - y_min, z_max - z_min], 'float32')
        print("array size:", np.shape(stack_array))

        stack_array[0, :, :, :] = raw_mask[x_min: x_max, y_min: y_max, z_min: z_max]
        stack_array[1, :, :, :] = model_output_gt[x_min: x_max, y_min: y_max, z_min: z_max]
        stack_array[2, :, :, :] = penalty_fn[x_min: x_max, y_min: y_max, z_min: z_max]
        stack_array[3, :, :, :] = penalty_fp[x_min: x_max, y_min: y_max, z_min: z_max]

        Functions.save_np_array(save_dict, fn, stack_array, compress=True)
        count += 1


def get_stack_array_256(semantic='artery', fold=(0, 1), cta=False, version='0', correct_annotation_example_dir=None):
    """

        form the stack array for each case (no lesion):
        array with shape [4, 256, 256, 256], each channel is: raw_mask, model_output_gt, penalty_fn, penalty fp

        :param correct_annotation_example_dir: like
        '/data_disk/artery_vein_project/extract_blood_region/training_data/sliced_sample/256_v0', only file name in
        similar directory will be considered as correct
        :param version:
        :param cta:
        :param fold:
        :param semantic:
    ...
    if cta:
        ...
        save_top_dict = '/data_disk/artery_vein_project/extract_blood_region/' \
                        'training_data/sliced_sample/256_v' + version + '/CTA/'
        ...
        data_top_dict = '/data_disk/artery_vein_project/new_data/non-contrast/'
        save_top_dict = \
            '/data_disk/artery_vein_project/extract_blood_region/' \
            'training_data/sliced_sample/256_v' + version + '/non-contrast/'
        if correct_annotation_example_dir is not None:
            correct_annotation_example_dir = os.path.join(correct_annotation_example_dir, 'non-contrast')

    assert semantic in ['artery', 'vein', 'blood']
    ...
    if semantic == 'artery':
        ...
        assert semantic == 'blood'
        save_dict = save_top_dict + 'stack_array_blood/'
        branch_dict = data_top_dict + 'depth_and_center-line/blood_branch_map/'
        vessel_mask_dict = data_top_dict + 'semantics/blood_mask/'
        depth_dict = data_top_dict + 'depth_and_center-line/depth_array/'
        if correct_annotation_example_dir is not None:
            correct_example_fn_dir_artery = os.path.join(correct_annotation_example_dir, 'stack_array_artery')
            correct_fn_list_artery = os.listdir(correct_example_fn_dir_artery)
            correct_example_fn_dir_vein = os.path.join(correct_annotation_example_dir, 'stack_array_vein')
            correct_fn_list_vein = os.listdir(correct_example_fn_dir_vein)
            correct_fn_list = list(set(correct_fn_list_artery) & set(correct_fn_list_vein))
            print("num correct file names", len(correct_fn_list))
    ...
    for fn in fn_list:
        ...
        raw_mask, model_output_gt, penalty_fn, _, penalty_fp = get_input_output_and_penalty_array(
            rescaled_ct_denoise, blood_branch_map, case_report, blood_vessel_mask, None, version=version)
...
if __name__ == '__main__':
    ...
    get_stack_array_256(cta=False, fold=(0, 4), version='1', semantic='blood',
